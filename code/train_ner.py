...
def train(args, train_dataset, model, tokenizer, labels, pad_token_label_id):
    ...
    for _ in train_iterator:
        ...
        if args.n_gpu == 1 and global_step > 500:  # and global_step % args.save_steps == 0:
            eval_results, _ = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode="dev",
                                       print_result=False)
...
def main():
    ...
    if args.do_train:
        ...
        global_step, tr_loss, max_f1, model_saved = train(args, train_dataset, model, tokenizer, labels,
                                                          pad_token_label_id)
        f1_step = 0.0
        if args.n_gpu == 1:
            eval_results, _ = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode="dev",
